...
<img src="assets/hex-godataR_nourl.png" align="right" height="260"/>
...
Users can also retrieve cases or laboratory records that fall within a specific date range, using `get_cases_epiwindow()` or `get_labresults_epiwindow()` respectively.

It is now possible to create a Go.Data bulk import-ready file of new or updated laboratory records matched to cases or existing lab records in Go.Data respectively, using the `lab2godata_wrapper()` function. If preferred, users not familiar with R can enter the parameters for this function by using the `lab2godata` shiny app.  The app can be launched from within the godataR package with `runlab2godata()`.  
