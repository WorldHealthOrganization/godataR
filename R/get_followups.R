#' Download contact follow-ups from Go.Data
#'
#' A function to retrieve the follow-up data for a
...
#'    Go.Data. This method relies on the GET outbreak/{id}/follow-ups
...
#'    outbreak/{id}/follow-ups/export API endpoint. An export
...
get_followups <- function(url=url,
  ...
  if (method == "batches") {
    ...
    api_call_n <- paste0(url, "api/outbreaks/",outbreak_id,"/follow-ups/filtered-count")
    api_call_get <- paste0(url, "api/outbreaks/",outbreak_id,"/follow-ups")
    ...
    api_call_request <- paste0(url,"api/outbreaks/",outbreak_id,"/follow-ups/export")
