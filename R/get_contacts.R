#' Download contacts from Go.Data
#'
...
#'    Go.Data. This method relies on the GET outbreak/{id}/contacts
...
#'    outbreak/{id}/contacts/export API endpoint. An export
...
get_contacts <- function(url=url,
  ...
  if (method == "batches") {
    ...
    api_call_n <- paste0(url, "api/outbreaks/",outbreak_id,"/contacts/filtered-count")
    api_call_get <- paste0(url, "api/outbreaks/",outbreak_id,"/contacts")
    ...
    api_call_request <- paste0(url,"api/outbreaks/",outbreak_id,"/contacts/export")
