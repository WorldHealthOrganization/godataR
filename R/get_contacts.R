...
#'
#' @return
#' Returns data frame of contacts. Some fields, such as addresses, hospitalization history, and questionnaire fields will require further unnesting. See the tidyr::unnest() function.
#' @export
#'
#' @examples
...
get_contacts <- function(url=url, username=username, password=password, outbreak_id=outbreak_id, batch_size=50000) {
  ...
  if (check_godata_version(url=url)==TRUE) {
    warning("Your version of Go.Data is 2.38.1 or later. Consider using the function get_contacts2().")
  }
  ...
  df_n <- GET(paste0(url,"api/outbreaks/",outbreak_id,"/contacts/filtered-count"),
  ...
  while (skip < df_n) {
    ...
    df.i <- GET(paste0(url,"api/outbreaks/",outbreak_id,"/contacts",
