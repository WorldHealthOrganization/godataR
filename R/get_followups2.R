#' Download follow-up of contacts from Go.Data (version 2.38.1 or later)
#'
#' A function to retrieve the follow-up data for
#' a specific `outbreak_id`. This function relies
#' on the `/outbreaks/{id}/follow-ups/export`
#' API endpoint.
#'
...
#'
#' @return
#' Returns data frame of follow-ups.
#' @export
#' @examples
#' \dontrun{
...
#'
#' followups <- get_followups2(url=url,
#'                             username=username,
#'                             password=password,
...
get_followups2 <- function(url=url,
                           username=username,
                           password=password,
                           outbreak_id=outbreak_id,
                           wait=5,
  ...
  if (file.type=="json") {
    request_id <- GET(paste0(url,"api/outbreaks/",outbreak_id,"/follow-ups/export",
