...
get_labresults2 <- function(url=url, username=username, password=password, outbreak_id=outbreak_id, wait=5, file.type=c("json","csv")) {
  ...
  if (file.type=="json") {
    request_id <- GET(paste0(url,"api/outbreaks/",outbreak_id,"/lab-results/export",
