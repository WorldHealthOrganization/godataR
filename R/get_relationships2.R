...
get_relationships2 <- function(url=url, username=username, password=password, outbreak_id=outbreak_id, wait=5) {

  #Check version of Go.Data
  if (check_godata_version(url=url)==FALSE) {
    stop("Go.Data must be version 2.38.1 or later. Please use the function get_relationships() instead.")
  }
  ...
  ##########################
  # Get Relationships Data Frame
  ##########################

  #Submit an export request to the system
  export.request <- GET(paste0(url,"api/outbreaks/",outbreak_id,"/relationships/export",
