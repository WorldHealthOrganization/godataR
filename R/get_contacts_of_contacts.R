#' Download contacts-of-contacts from Go.Data (version 2.38.0 or earlier)
#'
#' A function to retrieve the case data for a
#' specific `outbreak_id`. This function relies
#' on the `/outbreaks/{id}/contacts-of-contacts`
#' API endpoint. Records are imported in
#' iterative batches and then appended together
#' into a single data frame.
#'
#' This function works on all versions of Go.Data.
#' However, if you are using version 2.38.1 or
#' greater, then we recommend using \code{\link{get_contacts_of_contacts2}},
...
#' @param outbreak_id The id number for the outbreak for which you want to download contacts-of-contacts.
#' @param batch_size For large datasets, specify the number of records to retrieve in each iteration.
#'
#' @return
#' Returns a data frame of contacts-of-contacts. Some fields, such as addresses, hospitalization history, and questionnaire fields will require further unnesting. See the `tidyr::unnest()` function.
#' @export
#'
#' @examples
#' \dontrun{
...
get_contacts_of_contacts <- function(url=url,
  ...
  if (check_godata_version(url=url)==TRUE) {
    warning("Your version of Go.Data is 2.38.1 or later. Consider using the function get_contacts_of_contacts2().")
  }
  ...
  df_n <- GET(paste0(url,"api/outbreaks/",outbreak_id,"/contacts-of-contacts/filtered-count"),
  ...
  while (skip < df_n) {
    ...
    df.i <- GET(paste0(url,"api/outbreaks/",outbreak_id,"/contacts-of-contacts",
