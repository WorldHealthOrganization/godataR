...
#'    Go.Data. This method relies on the GET outbreak/{id}/events
...
#'    outbreak/{id}/events/export API endpoint. An export
...
get_events <- function(url=url,
  ...
  if (method == "batches") {
    ...
    api_call_n <- paste0(url, "api/outbreaks/",outbreak_id,"/events/filtered-count")
    api_call_get <- paste0(url, "api/outbreaks/",outbreak_id,"/events")
    ...
    api_call_request <- paste0(url,"api/outbreaks/",outbreak_id,"/events/export")
