...
#'
#' A function to retrieve the event data for a
#' specific `outbreak_id`. This function relies
#' on the `/outbreaks/{id}/events` API endpoint.
...
#' greater, then we recommend using \code{\link{get_events2}},
...
#'
#' @return
#' Returns data frame of events. may require further unnesting. See the `tidyr::unnest()` function.
#' @export
#'
#' @examples
#' \dontrun{
...
get_events <- function(url=url,
  ...
  if (check_godata_version(url=url)==TRUE) {
    warning("Your version of Go.Data is 2.38.1 or later. Consider using the function get_events2().")
  }
  ...
  df_n <- GET(paste0(url,"api/outbreaks/",outbreak_id,"/events/filtered-count"),
  ...
  while (skip < df_n) {
    ...
    df.i <- GET(paste0(url,"api/outbreaks/",outbreak_id,"/events",
